...
ContactInfo::ContactInfo(Contact l_contact, QWidget *parent) : QWidget(parent), ui(new Ui::ContactInfo), contact(l_contact) //Конструктор с определением контакта
{
    ...
    QHBoxLayout* VB_Buttons = ui->horizontalLayout;  //Берем список с кнопками
    VB_Buttons->setAlignment(Qt::AlignRight);     //Меняем настройку списка, для корректного отображения элементов
}
...
void ContactInfo::on_btn_red_clicked()
{
